...
function init() {
    ...
    hideButtons.remove();
    const observer = new MutationObserver(function(mutationsList, observer) { try { updateUI(); updatedChat(); } catch {} });
    try { updateUI() } catch {}
...
function openHaxballURL(uri) { const code = uri.replace(/^https?:\/\/(www\.)?haxball\.com\/play\?c=/, ""); if (code.length > 0) { window.location.replace("https://www.haxball.com/play?c=" + code); } }
...
function createAvatarMenu() {
    let avatarMenu = document.createElement("div");
    avatarMenu.setAttribute("class", "input-options");
    avatarMenu.setAttribute("hidden", "");
    avatarMenu.innerHTML = `
        <div class="dialog settings-view" style="height:min-content">
            <h1>Custom Avatar</h1>
            <button id="closeAvatar" style="position:absolute;top:12px;right:10px">Volver</button>
            <div style="text-align:center; padding:20px;">
                <input type="file" id="avatarUpload" accept="image/*" style="display:none">
                <button id="uploadTrigger">Subir Imagen</button>
                <div id="previewAvatar" style="margin:20px auto; width:100px; height:100px; border-radius:50%; background-color:#333; background-size:cover; border:2px solid white"></div>
            </div>
        </div>
    `;
    body.parentNode.appendChild(avatarMenu);

    const savedAvatar = localStorage.getItem("my_avatar");
    if(savedAvatar) {
        avatarMenu.querySelector('#previewAvatar').style.backgroundImage = `url(${savedAvatar})`;
    }

    avatarMenu.querySelector('#uploadTrigger').addEventListener('click', () => avatarMenu.querySelector('#avatarUpload').click());
    avatarMenu.querySelector('#avatarUpload').addEventListener('change', function(e) {
        const file = e.target.files[0];
        if (file) {
            const reader = new FileReader();
            reader.onload = function(event) {
                localStorage.setItem("my_avatar", event.target.result);
                avatarMenu.querySelector('#previewAvatar').style.backgroundImage = `url(${event.target.result})`;
            }
            reader.readAsDataURL(file);
        }
    });

    avatarMenu.querySelector('#closeAvatar').addEventListener('click', () => avatarMenu.setAttribute("hidden", ""));
}
...
function updateUI() {
    ...
    if (body.querySelector('.roomlist-view')) {
        ...
        if (!getByDataHook('avatarbtn')) createAvatarButton();
...
function createAvatarButton() {
    ...
    button.setAttribute("data-hook", "avatarbtn");
    button.innerHTML = '<div>Avatar</div>';
    button.addEventListener("click", function() {
        document.querySelector('.input-options[hidden]').removeAttribute("hidden");
    });
...
function setupGameUI() {
    ...
    if (firstTime) {
        ...
        chat.querySelector('input').addEventListener('blur', function() {
            inputStyle.display = 'none';
        });
        firstTime = false;
    }
...
function updateJoystick(touch) {
    ...
    switch (joystickValue) {
        case 0: emulateKeys("d"); break;
        case 1: emulateKeys("sd"); break;
        case 2: emulateKeys("s"); break;
        case 3: emulateKeys("sa"); break;
        case 4: emulateKeys("a"); break;
        case 5: emulateKeys("wa"); break;
        case 6: emulateKeys("w"); break;
        case 7: emulateKeys("wd"); break;
        default:
    }
}
